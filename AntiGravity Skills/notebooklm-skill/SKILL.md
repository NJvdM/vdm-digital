...
description: Comprehensive workflows combining NotebookLM's research, analysis, and content creation tools with AntiGravity's building and automation capabilities. Covers learning, business, content creation, research, productivity, and creative use cases.
---
...
- User wants to **research a topic deeply** and create content from it
- User wants to **build a course, briefing, or content package**
- User wants to **automate client onboarding or competitor analysis**
- User wants to **create audio, video, or visual content** from research
- User wants to use **NotebookLM tools** (research, notebooks, quizzes, flashcards, etc.)
- User says "research", "deep dive", "create a course", "briefing", "podcast", "study guide", etc.
...
The `notebooklm` MCP server must be configured and authenticated. Verify with:

```bash
source $HOME/.local/bin/env && nlm login --check
```

If not authenticated, run:

```bash
source $HOME/.local/bin/env && nlm login --clear
```
...
| `notebook_query` | Ask AI about existing sources |
| `research_start` | Deep/fast research (find new sources) |
...
| `source_get_content` | Get raw text from a source |
| `studio_create` | Create any artifact (audio, video, report, etc.) |
...
> 🎨 **Match Visual Style to Audience**
> `kawaii` for Gen Z, `classic` for executives, `whiteboard` for education.
